...
def main(command_line=None):
    ...
    file_parser.add_argument(
        "--data", action="store", help="The data file name"
    )
    ...
    # Получение значения переменной окружения через python-dotenv
    ...
    if not data_file:
        ...
        print(data_file)
    if not data_file:
        print("Нет файла с данными!", file=sys.stderr)
        sys.exit(1)

    # Загрузить всех людей из файла, если файл существует
    is_dirty = False
    if os.path.exists(data_file):
        people = load_workers(data_file)
    else:
        people = []
